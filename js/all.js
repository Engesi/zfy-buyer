...
//判断手机是安卓还是苹果，安卓是1，苹果是3
function setPhoneType () { 
  let ua = navigator.userAgent.toLowerCase();
  let type = ua.includes('iphone') ? '3' : '1';
...
function addPhone(phone){
	...
	let set = [...(new Set(phoneList))];//生成一个set对象给数组去重,把set对象转成数组
	localStorage.setItem('phoneList', JSON.stringify(set));
}
